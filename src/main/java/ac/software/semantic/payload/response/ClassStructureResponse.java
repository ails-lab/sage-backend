...
public class ClassStructureResponse {
	...
	public static ClassStructureResponse createFrom(ClassStructure cs, boolean includeDatatypes) {
		...
		if (cs.getRange() != null) {
			...
			for (Resource r : cs.getRange()) {
				if (!includeDatatypes) {
					String uri = r.getURI();
					if (uri.startsWith("http://www.w3.org/2001/XMLSchema#") || uri.startsWith(RDFVocabulary.NAMESPACE)) {
						continue;
					}
				}
		...
		if (cs.getChildren() != null) {
			...
			for (ClassStructure c : cs.getChildren().values()) {
				ClassStructureResponse cc = ClassStructureResponse.createFrom(c, includeDatatypes);
				children.add(cc);
			}
