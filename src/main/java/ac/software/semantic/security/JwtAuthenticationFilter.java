    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        try {
            ...
            if (StringUtils.hasText(jwt) && tokenProvider.validateToken(jwt)) {
                ...
                if (lastLoginOpt.isPresent() && !userDetails.allowMultiLogin()) {
                	long lastLogin = lastLoginOpt.get().getTime();
                    long tokenIssuedAt = tokenProvider.getIssuedAtFromJWT(jwt).getTime();

                    if (lastLogin/1000 != tokenIssuedAt/1000) {
                        throw new Exception("Token expired, User logged in from another device");
                    	// logger.error("Token expired, User logged in from another device");
                        // return;
                    }
                }
            
                userDetails.setRole(UserRoleType.valueOf(tokenProvider.getRoleFromJWT(jwt)));
