...
//        	throw new UsernameNotFoundException("User has not access to current database");
        ...
        Optional<UserRole> urOpt = roleRepository.findByDatabaseIdAndUserId(database.getId(), user.getId());
        
        UserPrincipal principal = new UserPrincipal(user, urOpt.orElse(null));
        if (role != null) {
        	principal.setRole(UserRoleType.valueOf(role));
        }
...
//        	throw new UsernameNotFoundException("User has not access to current database");
        ...
        Optional<UserRole> urOpt = roleRepository.findByDatabaseIdAndUserId(database.getId(), new ObjectId(id));
        
        return new UserPrincipal(user, urOpt.orElse(null));
    }
