	...
	public void statsPerProvider(String virtuoso) throws Exception {
		...
		for (String s : new String[] { "europeanafashion", "europeanfilmgateway", "photoconsortium", "judaica", "euscreen", "pagode" }) {
			...
			for (Dataset ds : datasetRepository.findByUserId(user.getId())) {
				for (AnnotatorDocument adoc : annotatorRepository.findByDatasetUuid(ds.getUuid())) {
					v += "<" + adoc.asResource(resourceVocabulary) + "> ";
				}

				for (PagedAnnotationValidation pav : pavRepository.findByDatasetUuid(ds.getUuid())) {
					v += "<" + pav.asResource(resourceVocabulary) + "> ";
				}

				for (FilterAnnotationValidation fav : favRepository.findByDatasetUuid(ds.getUuid())) {
					v += "<" + fav.asResource(resourceVocabulary) + "> ";
				}
